...
export async function renderHud(hud, html, token, searchText, doImageSearch, updateTokenImage, updateActorImage) {
  const hudSettings = game.settings.get('LightsHUD', 'hudSettings');

  if (!hudSettings.enableSideMenu) return;

  const tokenActor = game.actors.get(token.actorId);
  const disableIfTHW = game.settings.get('LightsHUD', 'disableSideMenuIfTHW');
  ...
  const sharedOnly = game.settings.get('LightsHUD', 'displayOnlySharedImages');
  ...
  if (tokenActor) {
    actorVariants = tokenActor.getFlag('LightsHUD', 'variants') || [];
  ...
  let tokenImageName = '';
  if (token.flags['LightsHUD'] && token.flags['LightsHUD']['name']) {
    tokenImageName = token.flags['LightsHUD']['name'];
  } else {
    tokenImageName = getFileName(token.img);
  }

  let imagesParsed = [];
  const tokenConfigs = (game.settings.get('LightsHUD', 'tokenConfigs') || []).flat();
  ...
  divR
    ...
    .on('keyup', (event) =>
      _onImageSearchKeyUp(event, hud, html, token, doImageSearch, updateTokenImage, updateActorImage),
    );
  if (userHasConfigRights) {
...
async function _onImageClick(event, tokenId, updateTokenImage, updateActorImage) {
  ...
  if (shiftKeyDown) {
    ...
    let tokenImageName = token.getFlag('LightsHUD', 'name');
...
function _onImageRightClick(event, tokenId) {
  ...
  if (token.actor) {
    ...
    let variants = tokenActor.getFlag('LightsHUD', 'variants') || [];
    ...
    tokenActor.unsetFlag('LightsHUD', 'variants');
    if (variants.length > 0) {
      tokenActor.setFlag('LightsHUD', 'variants', variants);
    }
