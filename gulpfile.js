...
const getManifest = () => {
	...
	if (fs.existsSync(`src`)) {
		...
		json.root = "../../rol/FoundryVTT/data/modules/lights-hud-ate";
	}
...
function buildTS() {
	return (
		gulp
			...
			.pipe(gulp.dest("../../rol/FoundryVTT/data/modules/lights-hud-ate"))
	);
}
 ...
 */
function buildCSS() {
	return gulp.src(`src/**/*.css`).pipe(gulp.dest("../../rol/FoundryVTT/data/modules/lights-hud-ate"));
}
 ...
 */
function buildLess() {
	return gulp.src(`src/**/*.less`).pipe(less()).pipe(gulp.dest("../../rol/FoundryVTT/data/modules/lights-hud-ate"));
}
...
const bundleModule = async () => {
	...
	return bsfy
		...
		.plugin(tsify)
		.bundle()
		.pipe(source(path.join("../../rol/FoundryVTT/data/modules/lights-hud-ate", `bundle.js`)))
		.pipe(buffer())
				...
				if (fs.lstatSync(p).isDirectory())
					recursiveFileSearch(p, (err, res) => {
						...
						for (const file of res) {
							const newFile = path.join(
								"../../rol/FoundryVTT/data/modules/lights-hud-ate",
					...
					console.log(
						`Copying file: ` +
							p +
							` to ` +
							path.join("../../rol/FoundryVTT/data/modules/lights-hud-ate", entity)
					);
					fs.copyFileSync(p, path.join("../../rol/FoundryVTT/data/modules/lights-hud-ate", entity));
				}
			}
		}
...
const buildWatch = () => {
	...
	gulp.watch(`src/**/*.ts`, { ignoreInitial: false, delay: 1000 }, gulp.series(buildTS));
...
/**
 * Remove built files from '../../rol/FoundryVTT/data/modules/lights-hud-ate' folder
 * while ignoring source files
 */
const clean = async () => {
	if (!fs.existsSync("../../rol/FoundryVTT/data/modules/lights-hud-ate")) {
		fs.mkdirSync("../../rol/FoundryVTT/data/modules/lights-hud-ate");
	}
    ...
    */
	...
	try {
		for (const filePath of files) {
			if (fs.existsSync(path.join("../../rol/FoundryVTT/data/modules/lights-hud-ate", filePath))) {
				// fs.unlinkSync(path.join('../../rol/FoundryVTT/data/modules/lights-hud-ate', filePath)); // MOD 4535992
				fs.rmSync(path.join("../../rol/FoundryVTT/data/modules/lights-hud-ate", filePath), {
					recursive: true,
					force: true,
				});
			}
...
const linkUserData = async () => {
	...
	try {
		if (
			fs.existsSync(path.resolve(`.`, "../../rol/FoundryVTT/data/modules/lights-hud-ate", `module.json`)) ||
		...
		let linkDir;
		if (config.dataPath) {
			console.log("Tengo", config.dataPath);
			if (!fs.existsSync(path.join(config.dataPath, `Data`)))
				throw Error(`User Data path invalid, no Data directory found`);
			console.log("Tengo 2");
			linkDir = path.join(config.dataPath, `Data`, destDir, name);
			console.log("Tengo 2", linkDir);
		} else {
